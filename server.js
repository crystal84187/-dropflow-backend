...
// OFFICIAL ALIEXPRESS SIGNATURE METHOD
// Source: AliExpress Open Platform docs
function computeSignature(secret, apiMethod, params) {
// Step 1: Sort all params alphabetically by key
const sortedKeys = Object.keys(params).sort();

// Step 2: Concatenate method + all key-value pairs
let strToSign = apiMethod;
for (const key of sortedKeys) {
strToSign += key + params[key];
}

// Step 3: Wrap with secret
strToSign = secret + strToSign + secret;

// Step 4: HMAC-SHA256, uppercase hex
return crypto
...
// Add method after signing
params.method = apiMethod;

const qs = new URLSearchParams(params).toString();
const apiRes = await fetch(`${ALI_API_URL}?${qs}`);
const data = await apiRes.json();

console.log('AliExpress raw response:', JSON.stringify(data).slice(0, 300));
